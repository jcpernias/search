...
- Available prices: all prices from that could be indexed, ie, all prices in unselective coverage treatments and those from indexed vendors in selective coverage treatments; `pavail`. Average market prices are based on this variable.
...
Compute average market prices and average prices paid by shoopers. Also, create the `groups` variable. It will be used later to test for trends. 
...
We use plots in order to find evidence on the presence of trends in the expected prices. We also use Kruskall-Wallis tests to test the hypothesis that the mean of expected prices is the same across all periods. In order to do so, we divide the sample in three groups of 10 periods each. We perform two tests. One tests the null hypothesis of the three periods having the same mean. The other tests the null that the mean of the first periods is the same than the mean of the last periods. Finally, we test that the pcrices are associated with a trend. We use the Kendall correlation coefficient in order to allow for possible nonlinear association.  
...
```{r}
kw1 <- markets %>% group_by(treat) %>% 
  do(tidy(kruskal.test(pavg ~ group, data = .))) %>%
  select(-one_of(c("method", "parameter")))
```


Kruskall-Wallis dropping the central periods:

```{r}
kw2 <- filter(markets, group != 2) %>% group_by(treat) %>% 
  do(tidy(kruskal.test(pavg ~ group, data = .))) %>%
  select(-one_of(c("method", "parameter")))
```
...
```{r}
trend_tests <- left_join(kw1, kw2, by = "treat") %>%
  rename(kw_all = statistic.x, p_val_all = p.value.x, 
         kw_drop = statistic.y, p_val_drop = p.value.y) %>%
  left_join(kend, by = "treat") %>%
  rename(kendall = estimate, p_valk_kendall = p.value)

kable(trend_tests, digits = 3)
```
...
```{r}
kw1 <- markets %>% group_by(treat) %>% 
  do(tidy(kruskal.test(pmin ~ group, data = .))) %>%
  select(-one_of(c("method", "parameter")))
```
...
```{r}
kend <- markets %>% group_by(treat) %>% 
  do(tidy(cor.test(~ pmin + period, method = "kendall", data = .))) %>%
  select(-one_of(c("method", "statistic", "alternative")))
```
