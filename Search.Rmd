...
Compute average market prices and average prices paid by shoppers. Also, create the `part` variable. It will be used later to test for trends. 
...
markets <- smpl %>% group_by(market_id) %>%
  ...
  mutate(part = as.integer(floor((period  - 1) / 10) - 1))
```
...
```{r}
mean_tests(left_join(filter(markets, part == 3),
                     mom_db, by = "treat"), 
           var = ~pavg - pavg_mean,
           by_var = ~treat) %>%
  kable(digits = 3)
...
```{r}
markets3 <- filter(markets, part == 3)
...
qksone <- function(alpha) {
  ...
  uniroot(foo, c(0.5, 10), tol = 1e-8)
}

Ka <- qksone(alpha)$root


plots_df <- filter(smpl, !is.na(pavail)) %>% 
  select(treat_id, period, p = pavail)

cv_df <- plots_df %>% group_by(treat_id) %>% 
  summarise(cv = Ka / sqrt(n()))

emp_dist <- function(data) {
  n <- NROW(data)
  cv <- Ka / sqrt(n) 
  dist <- ecdf(data$p)
  x <- knots(dist)
...
ecdf_df <- plots_df %>% 
  group_by(treat_id) %>% 
  do(as_data_frame(emp_dist(.)))

ggplot(data = ecdf_df, aes(x = x, y = y)) +
  geom_step() +
  geom_ribbon(aes(ymin = ylo, ymax = yhi, alpha = 1)) +
