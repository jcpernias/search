...
corr_test2 <- function (formula, data, subset, na.action, ...) 
{
...
corr_test_inner <- function(data, frml, method) {
  f_env(frml) <- environment()
  cor.test(frml, method = method, data = data)
}

corr_test <- function(data, by_var, frml, method = "kendall") {
  data %>% group_by_(by_var) %>% 
    do(tidy(corr_test_inner(data = ., frml, method = method))) %>%
...
trend_tests <- function(var, group, filter_frml) {
  ...
  # kendall_frml <- f_interp(~ uq(var))
  # kendall_frml <- ~ pavg + period
  # print(kendall_frml)
  kend <- corr_test(markets, ~treat, ~ pavg + period)
  kend
}

trend_tests(~pavg,  ~group, ~ group != 2)
```
...
trend_tests <- left_join(kw1, kw2, by = "treat") %>%
  ...
  rename(kendall = estimate, p_val_kendall = p.value)
